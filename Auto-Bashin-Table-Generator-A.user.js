...
var init = async function () {
    ...
    window.skillDB = $.csv.toObjects(skillDB_csv);
    //logger(skillDB);
...
var main = async function (current, all) {
    ...
    let dropDownParent = window.dropDownParent;
    let skillDB = window.skillDB;
    //유저가 설정한 시뮬 환경 저장용
    let userSelected = {
    ...
    */
    ...
    async function makeCompleteSkillDatas(skillElements, rarity, category = '') {
        ...
        for (let i = 0; i < skillElements.length; i++) {
            ...
            //그 외에는 계산
                ...
                if (typeof (skillData) !== 'undefined') {
                    if (["직선 주파", "앵글링×스키밍"].includes(skillData['스킬명(한섭 시뮬)'])) {
    ...
    // console.log(normalSkillElements);
    //고유기때 써먹으려고 분리
    let result_Inherit = [
...
async function filterStart(filterString) {
    await init();
    let race_all = window.dropDownParent["코스 장소"].querySelectorAll("li.el-select-dropdown__item");

    let results = [];
    for (const race of race_all) {
        await race.click();
        let length_all = window.dropDownParent["코스 종류 및 거리"].querySelectorAll("li.el-select-dropdown__item");
        for (const leng of length_all) {
            let result_dict = {
                "경기장": race.querySelector("span").innerText,
                "거리": leng.querySelector("span").innerText,
            };
            await leng.click();
            let result = await main(1, 1);
            for (const key of ["스킬명(한섭)", "마신", "BASETIME"]) {
                result_dict[key] = result[0][key];
            }
    ...
    await document.querySelector("#app > div.main-frame > form > div:nth-child(14) > div > div > div").click();
    let filename = window.dropDownParent['각질'].querySelector("ul > li.selected").innerText;
    downloadDictionaryArrayAsTSV(results, filename);
}
